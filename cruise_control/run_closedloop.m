...
p_lead = 10;
z_lead = 0.1;
C_lead = zpk(-z_lead,-p_lead,1.75*p_lead/z_lead);
...
Hur = minreal(C/(1 + C*P));
ur = lsim(Hur,r,t);
[num_ur,den_ur]=tfdata(Hur);

% Get the stepinfo to determine max overshoot, etc.
stepinfo(Hyr)

% Equilibrium points of y an u
y_eq = v_star;
u_eq = (beta_drag/k)*((v_star)^2);
...
% Transfer function data for controller
[num_con,den_con]=tfdata(C);
...
% Plot the saturated and unsaturated throttle angles
% figure
% plot(time.data,u_sat.data,'r--',time.data,u_bar.data,'b-');
% xlabel('Time(s)');
% ylabel('Magnitude of Throttle angle');
% legend('Saturated throttle angle','Unsaturated throttle angle');
% title('Saturated and unsaturated throttle angles vs time');

% Plot the output y and the reference r
figure
plot(time.data,y_out.data,'r--',time.data,r_out.data,'b-');
xlabel('Time(s)');
ylabel('Magnitude');
...
figure
plot(time.data,y_bar.data,'r--',time.data,r_bar.data,'b-');
xlabel('Time(s)');
