...
T_6 = 1400; % turbine inlet temperature
...
eta_cp = 0.82; % polytropic compressor efficiency
eta_tp = 0.85; % polytropic turbine efficiency

eff_cup = 0.9; % recuperator effectiveness
eff_cir = 0.88; % recirculator effectiveness
...
pi_ic = 0.985; % pressure drop across intercooler
pi_ec = 0.985; % pressure drop across evaporator
pi_gc = 0.985; % pressure drop across generator

pi_b = 0.965; % pressure drop across burner
...
pi_76 = (T_7/T_6)^((gam_h*eta_tp)/(gam_h-1)); % pressure drop across turbine
...
q_g = r*cp_m*(T_10 - T_91); % generator heat
q_e = (1+r)*cp_m*(T_32 - T_31); % evaporator heat

q_ext = (cop*q_e)-q_g; % external load
...
eff = (w + q_ext)/q_in; % total efficiency
